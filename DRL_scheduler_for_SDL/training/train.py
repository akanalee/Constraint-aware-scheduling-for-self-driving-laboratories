...
Decision epochs are triggered only when new jobs arrive in the buffer
(buffer watermark mechanism). Between decision epochs, time advances
until the next job completion.
"""
import os
import sys
import time
...
class FSSTrainer:
    ...
    def __init__(self, env_config_path, ppo_config=None):
        ...
        self.adapter = SDLEnvAdapter(self.env, ppo_config)
