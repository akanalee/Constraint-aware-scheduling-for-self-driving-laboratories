...
from models import JobActor, MachineActor
...
class PPO:
    """
    PPO Agent -
    : Job Actor + Machine Actor
    """
    def __init__(self, config: FSSConfig):
        self.config = config
    ...
    def select_action(self, state, greedy=False):
        ...
        Args:
            state:  'adj', 'fea', 'candidate', 'mask', 'mask_mch', 'dur', 'mch_time'
            greedy: greedy

        Returns:
            op_action: operation index
            mch_action: machine index
        """
        with torch.no_grad():
            #
            ...
            # Job Actor forward
            action, index, log_a, action_node, action_feature, mask_mch_action, hx, _ = self.policy_old_job(
                x=fea,
                graph_pool=g_pool,
                padded_nei=None,
                adj=adj,
    ...
    def update(self, memories, epoch):
        ...
        Returns:
            job_loss: job actor loss
            mch_loss: machine actor loss
        """
        # discounted rewards
        rewards_all_env = []
        ...
        for _ in range(self.k_epochs):
            ...
            for i in range(len(memories.fea_mb)):
                ...
                a_entropy, v, log_a, action_node, action_feature, mask_mch_action, hx = self.policy_job(
                    x=env_fea,
