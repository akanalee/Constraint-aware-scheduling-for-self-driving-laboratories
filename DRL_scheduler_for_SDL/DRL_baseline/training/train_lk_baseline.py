"""Conventional DRL baseline (Lei Kun) training script.

GIN input matches the original Lei Kun setting: input_dim=2,
node features = [lb/1000, is_scheduled], et_normalize_coef=1000.
All other training mechanics mirror the main MultiPPO trainer.
"""
import os
import sys
import time
...
# DRL_baseline/ (for models/, ppo.py, config.py, env_adapter.py)
project_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
# DRL_scheduler_for_SDL/ (for utils/, envs/, configs/)
sdl_root = os.path.dirname(project_root)
sys.path.insert(0, sdl_root)
...
from env_adapter import progressive_release_strict_fast as progressive_release_strict


class _NullWriter:
    """Null writer to suppress stdout from external functions without modifying them."""
    def write(self, *args, **kwargs): pass
    def flush(self, *args, **kwargs): pass


class Config:
    """Dict-to-attribute wrapper for config dicts."""
...
# Lei Kun baseline adapter: GIN input/normalization matches the original paper.

class LKSDLEnvAdapter(SDLEnvAdapter):
    """SDLEnvAdapter subclass with Lei Kun's node features:
      - input_dim = 2
      - fea = [lb / 1000, is_scheduled]
    All other tensors are inherited unchanged from the parent.
    """

    def get_state_for_multippo(self):
        state = super().get_state_for_multippo()
        if state is None:
            return None

        # Parent fea is 5-dim; collapse to Lei Kun's 2-dim [lb/1000, is_scheduled].
        fea5d = state['fea']
        lk_fea = torch.stack([
            fea5d[:, 0] * 0.1,
            fea5d[:, 1],
...
class MultiPPOTrainer:
    """MultiPPO trainer for the Lei Kun baseline."""
    def __init__(self, env_config_path, ppo_config=None):
        ...
        print(f"\nSDL System Configuration (LK Baseline):")
        ...
        ppo_config.n_j = self.env_config_raw.get('env_config', {}).get('n_jobs', 40)

        # Lei Kun baseline overrides.
        ppo_config.input_dim = 2
        ppo_config.et_normalize_coef = 1000.0

        self.ppo_config = ppo_config
        self.adapter = LKSDLEnvAdapter(self.env, ppo_config)
    ...
    def _validate_machine_mapping(self):
        """Sanity-check machine_id <-> idx maps."""
        assert len(self.adapter.machine_id_to_idx) == len(self.env.machines), \
            f"Machine mapping size mismatch: " \
            f"machines={len(self.env.machines)}, mapping={len(self.adapter.machine_id_to_idx)}"
    ...
    def _validate_operations(self):
        """Sanity-check ops have non-empty processing_times on known machines."""
        ...
        for op in self.env.graph.operations.values():
            ...
            for mach_id in op.processing_times.keys():
                if mach_id not in self.adapter.machine_id_to_idx:
                    raise ValueError(f"Unknown machine {mach_id} in op {op.id}.")

    def collect_episode(self, memory: Memory, episode_idx: int):
        """Roll out one episode; decision epochs fire only when buffer grows."""
        ...
        while not self.env._check_all_jobs_completed() and step_count < max_steps:
            ...
            _init_state = self.adapter.get_state_for_multippo()
            ...
            while len(self.env.graph.eligible_ops) > 0:
                ...
                state = self.adapter.get_state_for_multippo()
                ...
                action, a_idx, log_a, action_node, action_feature, mask_mch_action, hx, job_entropy = \
                ...
                pi_mch, pool = self.ppo.policy_old_mch(
                ...
                subproblem_schedule.append((start_time, op_id, machine_id, end_time))
                multippo_schedule.append((start_time, op_id, machine_id, end_time))
                ...
                reward = self.adapter.compute_reward(
                    ...
                    curr_makespan,
                    False,
                    scheduled_op_id=op_id,
                    scheduled_machine=machine_id
                )
    ...
    def _check_tmax_violations(self):
        """Count Tmax constraint violations in the current schedule."""
        violations = 0
    ...
    def train(self, num_episodes=1000, save_interval=1):
        """Training loop."""
        print(f"\n{'#'*60}")
        print(f"Training Start (LK Baseline, input_dim=2, et_normalize_coef=1000)")
        ...
        print(f"\n{'#'*60}")
        print(f"Training Completed (LK Baseline)")
        print(f"  Total episodes: {num_episodes}")
        print(f"  Final avg makespan (last 50): {np.mean(self.episode_makespans[-50:]):.2f}")
        print(f"{'#'*60}\n")


def main():
    # Point to the shared sdl_config.yaml (same as main training)
    _sdl_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    env_config_path = os.path.join(_sdl_root, 'configs', 'sdl_config.yaml')
    ...
    ppo_config.save_dir = './saved_models_lk'

    print(f"\n{'#'*60}")
    print(f"Initializing MultiPPO Trainer (LK Baseline) for SDL System")
    print(f"  GIN input: 2D [lb/1000, is_scheduled]  (Lei Kun original)")
    print(f"  et_normalize_coef: 1000  (Lei Kun original)")
    print(f"{'#'*60}\n")

    trainer = MultiPPOTrainer(env_config_path, ppo_config)

    print(f"\n{'#'*60}")
    print(f"Configuration Summary (LK Baseline):")
    ...
    print(f"  input_dim: {trainer.ppo_config.input_dim}  (Lei Kun 2D)")
    print(f"  et_normalize_coef: {trainer.ppo_config.et_normalize_coef}  (Lei Kun)")
